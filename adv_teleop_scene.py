...
It:
  1. Serves adv_teleop_scene.html over HTTP.
  ...
  4. Sends A/B/X/Y commands to the robot's advanced_test_teleop.py TCP server.
  5. Forwards continuous VR hand pose + joystick axes to Pi for arm/head control.
...
Then run on laptop:
    python3 adv_teleop_scene.py

Open on Quest:
    http://LAPTOP_IP:8080/adv_teleop_scene.html?v=advanced_head_gripper
"""
...
def run_http(http_host, http_port, directory, robot_camera_url):
    ...
    with http.server.ThreadingHTTPServer((http_host, http_port), handler) as httpd:
        ...
        print(f"[HTTP] Quest URL:          http://LAPTOP_IP:{http_port}/adv_teleop_scene.html")
        httpd.serve_forever()
...
class RobotTCPClient:
    ...
    def send_line(self, line, read_response=False):
        if self.token:
            line = f"{self.token} {line}"
        ...
        try:
            ...
            print(f"[ROBOT_TCP] failed to send: {line[:120]}... error={e}")
            return ""

    def send_command(self, cmd):
        return self.send_line(cmd, read_response=True)

    def send_vr_packet(self, packet):
        msg = "vr " + json.dumps(packet, separators=(",", ":"))
        return self.send_line(msg, read_response=False)
...
class VRRobotBridge:
    def __init__(
        ...
        self.last_vr_send_time = 0.0
        self.vr_enabled_sent = False

    async def handle_packet(self, packet):
        self.print_debug(packet)

        if not self.vr_enabled_sent:
            await asyncio.to_thread(self.client.send_command, "vr_on")
            self.vr_enabled_sent = True

        await self.handle_buttons(packet)
        await self.forward_vr_pose(packet)

    async def handle_buttons(self, packet):
        ...
        for button_name, robot_cmd in BUTTON_COMMANDS.items():
            ...
            if rising_edge and cooldown_ok:
                ...
                if resp:
                    print(f"[ROBOT] {robot_cmd} -> {resp.strip()}")

                self.last_command_time = time.time()

            self.last_buttons[button_name] = pressed

    async def forward_vr_pose(self, packet):
        now = time.time()

        if now - self.last_vr_send_time < self.vr_send_dt:
            return
        ...
        for hand in ("left", "right"):
            ...
            out = {
                "trigger": float(h.get("trigger", 0.0)),
                "grip": float(h.get("grip", 0.0)),
            }
        ...
        await asyncio.to_thread(self.client.send_vr_packet, small)
        self.last_vr_send_time = now
        ...
        if self.verbose_pose:
            ...
            for hand in ("left", "right"):
                ...
                parts.append(
                    ...
                    f"trig={trig:.2f} grip={grip:.2f} axes={axes}"
                )
...
def main():
    ...
    parser.add_argument("--vr-send-hz", type=float, default=15.0)
    ...
    print("[MAP] right joystick -> head pan/tilt")
    print("[MAP] grip + hand pose -> arm/end-effector approximate control")
    print("[MAP] trigger -> close gripper, release -> open gripper")
